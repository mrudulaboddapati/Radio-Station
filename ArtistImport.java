...
public class ArtistImport extends ImportBasePanel {
	...
	public void refreshData(final DefaultTableModel model) {
		new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() {
				...
				dbTabletoJTable.showdatainTable(model, "select * from artist");
				return null;
			}
		}.execute();
	}
	...
	public void start() {
		...
		final JFilePicker filePicker = new JFilePicker("Pick artist file");
				...
				SwingWorker<Void, Void> mySwingWorker = new SwingWorker<Void, Void>() {
					...
					protected Void doInBackground() {
						...
						try {
							...
							fileParser.artistFileParser(filePicker.textField
									.getText());
							...
							public void propertyChange(PropertyChangeEvent evt) {
								if (evt.getPropertyName().equals("state")) {
									if (evt.getNewValue() == SwingWorker.StateValue.DONE) {
										...
										refreshData(model);
										JOptionPane.showMessageDialog(ArtistImport.this,
												"Artist csv imported to database");
										dialog.dispose();
