...
public class AlbumImport extends ImportBasePanel {
	...
	public void refreshData(final DefaultTableModel model) {
		new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() {
				...
				dbTabletoJTable.showdatainTable(model, "select * from albums");
				return null;
			}
		}.execute();
	}
	...
	public void start() {
		...
		final JFilePicker filePicker = new JFilePicker("Pick album file");
				...
				SwingWorker<Void, Void> mySwingWorker = new SwingWorker<Void, Void>() {
					...
					protected Void doInBackground() {
						...
						try {
							...
							fileParser.albumsFileParser(filePicker.textField
									.getText());
							...
							public void propertyChange(PropertyChangeEvent evt) {
								if (evt.getPropertyName().equals("state")) {
									if (evt.getNewValue() == SwingWorker.StateValue.DONE) {
										...
										refreshData(model);
										JOptionPane.showMessageDialog(AlbumImport.this,
												"Album csv imported to database");
										dialog.dispose();
